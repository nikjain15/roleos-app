...
export default skill({
  ...
  gate: "full",
  structured: true,
  voiceCritic: false, // a résumé is the candidate's bullets, not RO's voice — truth gate governs
  prompt: ({ data }) => {
    ...
    return {
      system: [
        ...
        "profile (Company · Title · Dates). Keep the real company, title, and dates; never invent a",
        "role or merge two employers into one.",
        "COVERAGE — do NOT thin the résumé: include EVERY distinct real role/employer the master",
        "profile describes as its own section (do not drop or collapse a role), and give each role",
        "3–6 substantive tailored bullet lines drawn from that role's real achievements. Never reduce",
        "a role with many accomplishments to a single line — surface the strongest ones for this role.",
