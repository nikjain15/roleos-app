...
export default async function RolesPage() {
  ...
  return (
    <>
      <ExploreHeader crumbs={[{ label: "Index", href: "/the-index" }, { label: "Role types" }]} />
      <main className="mx-auto max-w-3xl px-6 py-12">
        ...
        <ul className="mt-8 space-y-1.5">
          {archetypes.map((a) => (
            <li key={a.slug}>
              <Link href={`/the-index/role/${a.slug}`} className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-surf2">
