...
function LoginInner() {
  ...
  async function github() {
    ...
    if (error) setErr("Couldn't reach GitHub just now. Try the email link instead?");
  }
    ...
    <main className="mx-auto max-w-md px-6 py-16 sm:py-20">
      ...
      {sent ? (
        ...
        <div className="mt-7 space-y-3">
          {/* Primary — Google unlocks the most (Gmail + Calendar scopes RO needs
              later for Gate 2); it also converts best, so it gets the emphasis. */}
          <button
            onClick={google}
          ...
          {/* Secondaries — relevant but not primary, so they pair up in a row
              rather than stacking as more full-width buttons. */}
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={linkedin}
              aria-label="Continue with LinkedIn"
              className="flex min-h-11 items-center justify-center gap-2 rounded-md border border-bd bg-surf px-4 text-sm font-medium text-tx hover:bg-surf2"
            >
