 ...
 * (profile → More → Save to PDF) and a CV (PDF or plain text). Extracted text is
 * run through normalizeProfileText to strip boilerplate (fewer tokens, cleaner
 * signal). Image-only scanned PDFs yield little text → the onboarding
 * thin-input guard catches that honestly downstream.
 */

export { normalizeProfileText };
...
export async function extractDocumentText(file: File): Promise<string> {
  ...
  if (name.endsWith(".txt") || file.type === "text/plain") {
    return normalizeProfileText(await file.text());
  }
  ...
  if (name.endsWith(".pdf") || file.type === "application/pdf") {
    ...
    return normalizeProfileText(Array.isArray(text) ? text.join("\n") : text);
  }
