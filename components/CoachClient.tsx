...
import VoiceControls from "@/components/VoiceMode";
import { deliveryNotes, type CandidateTurn } from "@/lib/voice-metrics";
 ...
 *
 * X8 (flag passed by the server shell): voice mode — browser STT/TTS transport
 * over the SAME mock loop; captions stay on screen; delivery notes are computed
 * client-side from the transcript. No audio ever leaves the browser.
 */
type Prep = {
  panel_focus?: string[];
  format?: string;
...
function CoachInner({ voiceEnabled }: { voiceEnabled: boolean }) {
  ...
  // X8: voice mode is the user's choice on top of the server flag.
  ...
  async function mockTurn(message?: string, durationMs?: number) {
    setBusy("…");
    if (message) {
      setTranscript((t) => [...t, { role: "candidate", text: message }]);
      candidateTurns.current.push({ text: message, durationMs });
    }
    const res = await fetch("/api/coach", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <main className="mx-auto max-w-2xl px-6 py-12">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {voiceEnabled && (
              <button
                ...
                className="rounded-md border border-bd px-2.5 py-1 text-[11px] font-medium text-tx2 hover:bg-surf"
              >
                {voiceOn ? "Voice mode · on" : "Voice mode · off"}
              </button>
            )}
          </div>
          {transcript.length === 0 ? (
            ...
            <div className="mt-3 space-y-3">
              ...
              <VoiceControls
                active={voiceEnabled && voiceOn}
                interviewerLine={
                  transcript.length && transcript[transcript.length - 1].role === "interviewer"
                    ? transcript[transcript.length - 1].text
                    : null
                }
                disabled={!!busy}
                onAnswer={(text, durationMs) => mockTurn(text, durationMs)}
              />
              <textarea
                value={answer}
      ...
      {debriefData && (
        <section className="mt-6 rounded-xl border border-bd bg-surf p-5">
          ...
          {/* X8: delivery, from the words you actually said — computed in your
              browser from the transcript above, never from audio. */}
          {deliveryNotes(candidateTurns.current).length > 0 && candidateTurns.current.length > 0 && (
            <div className="mt-3">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-tx3">
                Delivery — from your transcript
              </p>
              <ul className="mt-1 space-y-1.5 text-sm text-tx2">
                {deliveryNotes(candidateTurns.current).map((n, i) => (
                  <li key={i}>{n}</li>
                ))}
              </ul>
            </div>
...
export default function CoachClient({ voiceEnabled }: { voiceEnabled: boolean }) {
  return (
    <Suspense>
      <CoachInner voiceEnabled={voiceEnabled} />
    </Suspense>
  );
}
